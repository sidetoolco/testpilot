...
const getProductStyling = (isCompetitor: boolean) => {
  const baseClasses =
    'mb-3 p-3 bg-white rounded border-l-4 cursor-pointer hover:bg-gray-50 transition-colors';
...
const ChosenProductCard: React.FC<ChosenProductCardProps> = ({
  ...
  return (
    <div className={styling.containerClass} onClick={() => onProductClick(product)}>
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-600">${product.price?.toFixed(2) || 'N/A'}</p>
        </div>
      </div>
      <p className={styling.textClass}>✓ Chosen Product</p>
    </div>
  );
};
