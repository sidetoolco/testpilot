export const RangeInput: React.FC<{ name: string; value: number; onChange: (e: any) => void }> = ({
  name,
  value,
  onChange,
}) => {
  ...
  return (
    <div className="flex justify-around flex-col md:flex-row items-start w-full">
      {labels.map((label, index) => (
        <label
          key={index}
          className="flex md:flex-col flex-row md:items-center justify-items-center space-y-1 md:space-y-0 md:space-x-2"
        >
          <input
            type="radio"
            name={name}
