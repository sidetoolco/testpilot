...
export const testService = {
  ...
  // Nueva función para crear test inicial con solo el objetivo
  async createInitialTest(objective: string) {
    try {
      const {
        data: { user },
      } = await supabase.auth.getUser();
      ...
      if (error) {
        throw new TestCreationError('Failed to create initial test', { error });
      }

      return test;
    } catch (error) {
      console.error('Initial test creation error:', error);
      if (error instanceof TestCreationError) {
        toast.error(error.message);
      } else {
        toast.error('An unexpected error occurred');
  ...
  // Nueva función para actualizar el step del test
  async updateTestStep(testId: string, step: string) {
    try {
      const { data: test, error } = await supabase
        .from('tests')
        .update({ step: step } as any)
        .eq('id', testId as any)
        .select()
        .single();

      if (error) {
        throw new TestCreationError('Failed to update test step', { error });
      }

      return test;
    } catch (error) {
      console.error('Test step update error:', error);
      if (error instanceof TestCreationError) {
        toast.error(error.message);
      } else {
        toast.error('An unexpected error occurred');
