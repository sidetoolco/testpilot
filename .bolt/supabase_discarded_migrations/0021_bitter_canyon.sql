/*
  ...
  1. New Tables
    - test_sessions: Main test information
...
-- Enable RLS on test_sessions first
...
CREATE POLICY "Users can view test competitors"
  ...
  USING (
    EXISTS (
      ...
      WHERE test_sessions.id = test_competitors.test_id
      AND EXISTS (
        SELECT 1 FROM public.profiles
        WHERE profiles.company_id = test_sessions.company_id
        AND profiles.id = auth.uid()
...
CREATE POLICY "Users can view test variations"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.test_sessions
      WHERE test_sessions.id = test_variations.test_id
      AND EXISTS (
        SELECT 1 FROM public.profiles
        WHERE profiles.company_id = test_sessions.company_id
        AND profiles.id = auth.uid()
...
CREATE POLICY "Users can view test demographics"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.test_sessions
      WHERE test_sessions.id = test_demographics.test_id
      AND EXISTS (
        SELECT 1 FROM public.profiles
        WHERE profiles.company_id = test_sessions.company_id
        AND profiles.id = auth.uid()
